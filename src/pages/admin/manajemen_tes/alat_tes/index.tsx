...
function AlatTes(props: snackbarProps) {
    ...
    const data = [
        {
            id: 'TEST-1234566890',
            title: 'Intelligenz Struktur Test (IST)',
            price: 100000,
            access: 'Semua',
            report_access: 'Pendidikan & Perusahaan',
            status: 'active',
        }
    ]
    ...
    const columns: MRT_ColumnDef<any>[] = [
        ...
        },
        {
            ...
            Cell: ({ row }) => {
                return <Typography>{Currency({ value: row.original.price })}</Typography>
            },
            filterFn: 'equals',
            filterSelectOptions: !data ? [''] : (Array.from(new Set(data.map(data => data.price.toString())))).map(data => data),
            filterVariant: 'select',
            muiFilterTextFieldProps: {
                variant: 'outlined',
            }
        },
        {
            ...
            Cell: ({ row }) => {
                return <Typography>{row.original.access}</Typography>
            },
            filterFn: 'equals',
            filterSelectOptions: !data ? [''] : (Array.from(new Set(data.map(data => data.access)))).map(data => data),
            filterVariant: 'select',
            muiFilterTextFieldProps: {
                variant: 'outlined',
            }
        },
        {
            ...
            Cell: ({ row }) => {
                return <Typography>{row.original.report_access}</Typography>
            },
            filterFn: 'equals',
            filterSelectOptions: !data ? [''] : (Array.from(new Set(data.map(data => data.report_access)))).map(data => data),
            filterVariant: 'select',
            muiFilterTextFieldProps: {
                variant: 'outlined',
            }
        ...
        },
        {
            ...
            Cell: ({ row }) => {
                return <div className='flex gap-2'>
                    <IconButton className="border-solid border-2 text-secondary-500" aria-label="confirm" onClick={() => navigate(`/manajemen-tes/alat-tes/edit/${row.original.id}`)}>
                        <RiEditCircleLine />
                    </IconButton>
            ...
            Cell: ({ row }) => {
                ...
                return <div id={row.index.toString()} key={row.index}>
                    ...
                    {!collapse ? <div className="mt-4">
                        ...
                        <div className="mt-4">
                            <div className="font-medium">
                                Akses Contoh Laporan
                            </div>
                            <div className="font-normal mt-2">
                                {row.original.report_access}
                            </div>
                        </div>
                        <div className="mt-4">
                            <div className="font-medium">
        ...
        <Layout>
            ...
            <Grid2 container justifyContent={'space-between'}>
                ...
                <Grid2 container gap={2}>
                    <Button className="w-full mt-4 bg-primary-600 text-base-white rounded-lg py-4 px-6 hover:bg-primary-600 md:mt-0 md:w-auto gap-2" onClick={() => navigate('/manajemen-tes/alat-tes/create')}><RiAddLine /> Tambah</Button>
                </Grid2>
            </Grid2>
