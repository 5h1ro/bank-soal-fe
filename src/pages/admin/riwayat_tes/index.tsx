...
function RiwayatTesPeserta(props: snackbarProps) {
            ...
            Cell: ({ row }) => {
                return <Grid2 container className="flex gap-2">
                    <Button className="border-solid border-2 text-primary-500 rounded-lg pl-4" aria-label="confirm" onClick={() => window.open(row.original.result, '_blank')}>
                        Hasil Tes <RiArrowDropDownLine className="ml-1"></RiArrowDropDownLine>
                    </Button>
                </Grid2>
            },
            filterFn: 'equals',
            filterSelectOptions: !data ? [''] : (Array.from(new Set(data.map(data => UppercaseFirstWord(data.result))))).map(data => data),
            filterVariant: 'select',
            muiFilterTextFieldProps: {
                variant: 'outlined',
            }
        },
        {
            ...
            Cell: ({ row }) => {
                return <div className='flex gap-2'>
                    <IconButton className="border-solid border-2 text-primary-500" aria-label="confirm" onClick={() => navigate(`/riwayat-tes/${row.original.id}`)}>
                        <RiEyeLine />
                    </IconButton>
                </div>
            },
...
export default RiwayatTesPeserta 