...
function Mutasi(props: snackbarProps) {
    ...
    const columns: MRT_ColumnDef<any>[] = [
        ...
        },
        {
            ...
            Cell: ({ row }) => {
                return <Typography className="text-success-500">{Currency({ value: row.original.nominal })}</Typography>
            },
            filterFn: 'equals',
        ...
        },
        {
            ...
            Cell: ({ row }) => {
                return <Typography>{Currency({ value: row.original.start })}</Typography>
            },
            filterFn: 'equals',
            filterSelectOptions: !data ? [''] : (Array.from(new Set(data.map(data => Currency({ value: data.start }))))).map(data => data),
            filterVariant: 'select',
            muiFilterTextFieldProps: {
                variant: 'outlined',
            }
        },
        {
            ...
            Cell: ({ row }) => {
                return <Typography>{Currency({ value: row.original.end })}</Typography>
            },
            filterFn: 'equals',
            filterSelectOptions: !data ? [''] : (Array.from(new Set(data.map(data => Currency({ value: data.end }))))).map(data => data),
            filterVariant: 'select',
            muiFilterTextFieldProps: {
                variant: 'outlined',
            }
                ...
                return <div id={row.index.toString()} key={row.index}>
                    ...
                    {!collapse ? <div className="mt-4">
                        ...
                        <div className="mt-4">
                            ...
                            <div className="font-normal mt-2">
                                <Typography className="text-success-500">{Currency({ value: row.original.nominal })}</Typography>
                            </div>
                        </div>
                        <div className="mt-4">
                            <div className="font-medium">
                            ...
                            <div className="font-normal mt-2">
                                {Currency({ value: row.original.start })}
                            </div>
                        </div>
                        <div className="mt-4">
                            <div className="font-medium">
                            ...
                            <div className="font-normal mt-2">
                                {Currency({ value: row.original.end })}
                            </div>
                        </div>
                    </div> : null}
