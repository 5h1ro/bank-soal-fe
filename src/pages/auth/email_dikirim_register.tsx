...
import { useRequestVerifyAccountMutation } from '../../api/auth.api';
import { snackbarType } from '../../interfaces/snackbar.interface';

const EmailDikirimRegister = () => {
    const { email } = useParams();
    const navigate = useNavigate();
    const [resend, { isLoading }] = useRequestVerifyAccountMutation();
                    ...
                    <Grid container marginX={'auto'} marginY={'auto'}>
                        <Card sx={{ width: '750px', minHeight: '80.665vh', boxShadow: 'none' }} className='flex items-center justify-center'>
                            <CardContent className='w-[84%]'>
                                ...
                                <Typography align="center" className='text-[36px] font-semibold'>
                                    Lakukan Verifikasi Akunmu
                                </Typography>
                                <Typography align="center" className='text-base font-normal text-general-500 mt-6'>
                                ...
                                <Grid marginTop={'24px'}>
                                    ...
                                    <Grid className='w-full justify-center flex'>
                                        {!isLoading && (
                                            <Button type="submit" className='mt-6 text-lg text-base-white bg-primary-500 hover:bg-primary-500 font-medium rounded-xl p-4 shadow-none hover:shadow-none' variant="contained" onClick={async () => {
                                                try {
...
export default EmailDikirimRegister